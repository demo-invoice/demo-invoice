 ...
 *
 * If this context is ever migrated to a useReducer pattern, all action type
 * strings MUST be declared here before being dispatched (see T7 lesson —
 * dispatching unrecognised string action types causes silent state corruption).
 *
 * T24 (Send Invoice by Email): send state is intentionally managed as local
 * component state inside SendEmailModal — NO SEND_EMAIL action is dispatched
 * to this context. If future iterations need global send tracking (e.g. audit
 * log, optimistic UI), add a typed action constant here first, then handle it
 * in the reducer. Do NOT dispatch raw strings.
 * ─────────────────────────────────────────────────────────────────────────────
 */
